...
class DataSet:
    def __init__(self, train_src, valid_src=None, test_src=None, batch_size=32, n_ident=None, g_ident=None, l_ident=None):
        ...
        if type(train_src) == str:
            ...
            self.read_datasets(test_src, train_src, valid_src)
    ...
    def read_datasets(self, test_srcs, train_srcs, valid_srcs):
        ...
        if valid_srcs is None and test_srcs is None:
            random.shuffle(self.train_examples)
            self.valid_examples = self.train_examples[int(0.8*len(self.train_examples)):int(len(self.train_examples))]
            self.test_examples = self.train_examples[int(0.8*len(self.train_examples)):int(len(self.train_examples))]
            self.train_examples = self.train_examples[:int(0.8*len(self.train_examples))]
